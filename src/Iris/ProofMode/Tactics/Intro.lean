...
private partial def iIntroCore {prop : Q(Type u)} {bi : Q(BI $prop)}
  ...
  | (ref, .intro pat) :: pats =>
    ...
    | .intuitionistic pat, some _ =>
      ...
      let pf ← iCasesCore bi hyps A2 pat q(true) q(iprop(□ $B)) B ⟨⟩
        (fun {P} (hyps : Hyps bi P) (goal : Q($prop)) => iIntroCore hyps goal pats)
    ...
    | _, some _ =>
      ...
      let pf ← iCasesCore bi hyps A2 pat q(false) B B ⟨⟩
        (fun {P} (hyps : Hyps bi P) (goal : Q($prop)) => iIntroCore hyps goal pats)
      return q(imp_intro_spatial (Q := $Q) $pf)
    | _, none =>
      let .some _ ← ProofModeM.trySynthInstanceQ q(FromWand $Q $A1 $A2)
        | throwError "iintro: {Q} not a wand"
      let pf ← iCasesCore bi hyps A2 pat q(false) A1 A1 ⟨⟩
        (fun {P} (hyps : Hyps bi P) (goal : Q($prop)) => iIntroCore hyps goal pats)
