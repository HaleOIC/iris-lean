...
theorem wp_wand m E (e : Exp) (Φ Ψ : Val → IProp GF) :
  WP e @ m; E { { Φ } } -∗
  (∀ v, Φ v -∗ Ψ v) -∗
  WP e @ m; E { { Ψ } } := by
    simp only [wp_unfold]
...
theorem wp_atomic m E1 E2 (e : Exp) (Φ : Val → IProp GF) :
...
theorem fupd_wp m E (e : Exp) (Φ : Val → IProp GF) :
  (|={E}=> WP e @ m; E { { v, Φ v } }) ⊢ WP e @ m; E { { Φ } } := by
    simp only [wp_unfold]
    iapply (fupd_wpi _).2

theorem wp_fupd m E (e : Exp) (Φ : Val → IProp GF) :
...
theorem wp_val m E (v : Val) (Φ : Val → IProp GF) :
    Φ v -∗
    WP hl(v(v)) @ m; E { { Φ } } := by
